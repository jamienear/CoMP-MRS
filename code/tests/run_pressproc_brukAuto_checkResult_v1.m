% run_pressproc_brukAuto_mepacrine.m
% Jamie Near, McGill University 2014.
% 
% USAGE:
% [out,out_w,out_noproc,out_w_noproc]=run_pressproc_brukAuto_mepacrine(filestring,filestringw,aaDomain,tmaxin,iterin);
% 
% DESCRIPTION:
...
function [out,outw,out_noproc,outw_noproc]=run_pressproc_brukAuto_checkResult_v1(filestring,aaDomain,tmaxin,iterin);
...
%Since this study uses PV6 with "reference scans" turned on, the wate
%reference will always be acquired.  
water='y';
...
[raw,raww]=io_loadspec_bruk(filestring);
% if water
...
%combine coil channels if necessary raw bruker data.
if raw.dims.coils==0
    out_cc=raw;
elseif raw.dims.coils
    %Combining RF channels:
    disp('Combining RF channels:');
    [out_cc,fids_presum,specs_presum,phs,sigs]=op_addrcvrs(raw,1,'w');
end
%Water data already has the coils combined.
if water
    outw_cc=raww;
end
...
if driftCorr=='n' || driftCorr=='N'
    ...
    while sat=='n' || sat=='N'
        ...
        while (abs(fsPoly(1))>0.001 || abs(phsPoly(1))>0.01) && iter<iterin
            ...
            switch aaDomain
                case 't'
                    [out_aa,fs,phs]=op_alignAverages(out_rm2,0.2);  %**JN** 20190425 Force to use 0.2 sec in time domain for spectral registration.
                case 'f'
...
% Check to see if processed data versus unprocessed data has better FWHMax
% and height. Whichever is best will be written into LCModel format,
% (Peak Height/FWHMax, want largest number) 
% First try NAA
hNAA_proc=op_getPeakHeight(out,1.8,2.2);
hNAA_noproc=op_getPeakHeight(out_noproc,1.8,2.2);
FWHMaxNAA_proc=op_getLW(out,1.8,2.2);
FWHMaxNAA_noproc=op_getLW(out_noproc, 1.8, 2.2);
RatioProcNAA=hNAA_proc/FWHMaxNAA_proc;
RatioNoProcNAA=hNAA_noproc/FWHMaxNAA_noproc;

% For Creatine
hCr_proc=op_getPeakHeight(out,2.9,3.1);
hCr_noproc=op_getPeakHeight(out_noproc,2.9,3.1);
FWHMaxCr_proc=op_getLW(out,2.9,3.1);
FWHMaxCr_noproc=op_getLW(out_noproc,2.9,3.1);
RatioProcCr=hCr_proc/FWHMaxCr_proc;
RatioNoProcCr=hCr_noproc/FWHMaxCr_noproc;

% For Water
hW_proc=op_getPeakHeight(out,4.5,4.9);
hW_noproc=op_getPeakHeight(out_noproc,4.5,4.9);
FWHMaxW_proc=op_getLW(out,4.5,4.9);
FWHMaxW_noproc=op_getLW(out_noproc,4.5,4.9);
RatioProcW=hW_proc/FWHMaxW_proc;
RatioNoProcW=hW_noproc/FWHMaxW_noproc;

% Average Ratio
MeanRatioProc=mean([RatioProcNAA,RatioProcCr,RatioProcW]);
MeanRatioNoProc=mean([RatioNoProcNAA,RatioNoProcCr,RatioNoProcW]);
...
if writ=='y' || writ=='Y'
    ...
    disp(['MeanRatioProc = ' num2str(MeanRatioProc)]);
    disp(['MeanRatioNoProc = ' num2str(MeanRatioNoProc)]);
    
    if MeanRatioProc > MeanRatioNoProc
        disp(['Process spectrum will be written to LCModel']);
        writeText='Processed';
        [~]=io_writelcm(out,[filestring '/press/press_lcm'],out.te);
    else 
        [~]=io_writelcm(out_noproc,[filestring '/press/press_lcm'],out_noproc.te);
        disp(['Unprocessed spectrum will be written to LCModel']);
        writeText='Unprocessed';
    end
...
fprintf(fid,'\n\n<p>Peak Height to FWHMax ratio (MeanRatioProc) for processed data = %2.5f</p>',MeanRatioProc);
fprintf(fid,'\n<p>Peak Height to FWHMax ratio (MeanRatioNoProc) for unprocessed data = %2.5f</p>',MeanRatioNoProc);
fprintf(fid,'\n<p>%s Spectrum written to LCModel format</p>',writeText);
fclose(fid);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
